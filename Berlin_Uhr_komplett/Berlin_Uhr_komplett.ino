...
*Uhrzeit ermiteln und auf einer LED-Matrix in Form der Berlin-Uhr darstellen *
...
*othek angesteuert.                                                          *
*Die Sekungen werden über eine im 2Sekundentakt blinkende LED dargestellt.   *
...
// Bei ungerade Sekunden die Sekundenleuchte Aus bei geraden einschalten
  if (GvSek%2 == 0) {
    gr_lc.setLed(0,6,5,true);
  } else {
    gr_lc.setLed(0,6,5,false);
  }
...
void setLEDStd() {
  ...
  Gv5StdAnz = GvStd / 5; 
  Gv1StdAnz = GvStd % 5;
  for (idx = 1; idx < (Gv1StdAnz + 1); idx++) {
    gr_lc.setLed(0,idx,4,true);
  }
  for (idx = 1; idx < (Gv5StdAnz + 1); idx++) {
...
// Setzen der Zeile 3
  for (idx = 1; idx < (Lv5MinAnzB + 1); idx++) {
